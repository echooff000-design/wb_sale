// --- PROXIED DIRECT SHAREPOINT ENDPOINT ---
const SHAREPOINT_RAW = "https://tilaknagarindustries-my.sharepoint.com/:x:/g/personal/andebnath_tilind_com/IQBQOO7YAaZLQIlMbx55OXr1AdvPSbB0XsQEEmRvQMvFdBY?download=1";
const SYNC_PROXY_URL = "https://api.allorigins.win/raw?url=" + encodeURIComponent(SHAREPOINT_RAW);
...
// --- DIRECT FETCH & XLSB PARSER ---
async function syncLiveSharePoint() {
    const indicator = document.getElementById("syncIndicator");
    indicator.innerText = "🔄 Downloading Live Data...";

    try {
        const res = await fetch(SYNC_PROXY_URL);
        if (!res.ok) throw new Error("Sync failed");
        
        const arrayBuffer = await res.arrayBuffer();
        parseExcelBinary(arrayBuffer);
        indicator.innerText = "● Cloud Synced!";
    } catch (err) {
        console.warn("Direct proxy failed, loading local cache", err);
        indicator.innerText = "⚠️ Offline Cached";
...
function parseExcelBinary(buffer) {
    ...
    if (usersData.length > 0) {
        const rawDate = usersData[0]["Date"] || usersData[0]["date"] || usersData[0]["__EMPTY_5"] || "";
        const dayMatch = String(rawDate).match(/\b(\d{1,2})\b/);
    ...
    outletMaster.forEach(r => {
        ...
        if (lic) {
            metaMap[lic] = {
                ...
                zone: r["Zone"] || "West Bengal",
                asm: r["ASM"] || "Unassigned",
                tse: r["TSE"] || "Unassigned"
            };
        }
    });
    ...
    function processSheet(arr, key) {
        arr.forEach(r => {
            ...
            if (!combined[uid]) {
                const meta = metaMap[lic] || { group: "Unassigned", zone: "West Bengal", asm: r["ASM"] || "Unassigned", tse: r["TSE"] || "Unassigned" };
                combined[uid] = {
                    lic: lic,
...
function renderHierarchies(data) {
    ...
    h1 += rowH1('West Bengal', data, 'grand-total-row', 6);
    h2 += rowH2('West Bengal', data, 'grand-total-row', 6);
    h3 += rowH3('West Bengal', data, 'grand-total-row', 6);

    [...new Set(data.map(d => d.zone).filter(Boolean))].sort().forEach(z => {
        const zD = data.filter(d => d.zone === z);
        h1 += rowH1(z, zD, 'subtotal-row', 6);
        h2 += rowH2(z, zD, 'subtotal-row', 6);
        h3 += rowH3(z, zD, 'subtotal-row', 6);

        [...new Set(zD.map(d => d.asm).filter(Boolean))].sort().forEach(a => {
            const aD = zD.filter(d => d.asm === a);
            h1 += rowH1(a, aD, 'subtotal-row', 14);
            h2 += rowH2(a, aD, 'subtotal-row', 14);
            h3 += rowH3(a, aD, 'subtotal-row', 14);
...
function runAskAssistant() {
    ...
    if (qType.includes("Daily Run")) {
        ...
        countLabel.innerText = `Basis: ${lastDaysElapsed} Days Elapsed`;
        return;
    }
...
function downloadAskReportExcel() {
    if (!currentQueriedDataForExcel.length) return;
